...
//---------------------------------------------------------------------------
bool TSnapBase_LittleEndian;
void TSnapBase_init()
{
	int x = 1;
	TSnapBase_LittleEndian=*(char *)&x == 1;
}
//---------------------------------------------------------------------------
word TSnapBase_SwapWord(word Value)
{
	if (TSnapBase_LittleEndian)
		return  ((Value >> 8) & 0xFF) | ((Value << 8) & 0xFF00);
	else
			return Value;
}
//---------------------------------------------------------------------------
longword TSnapBase_SwapDWord(longword Value)
{
	if (TSnapBase_LittleEndian)
...
void TMsgSocket_GetLocal()
{
		...
		if (getsockname(FSocket, (struct sockaddr*)&LocalSin, &namelen)==0)
				LocalPort=TMsgSocket_GetSin(LocalSin, LocalAddress, LocalPort);
}
//---------------------------------------------------------------------------
...
//    #endif
//	if (getpeername(FSocket,(struct sockaddr*)&RemoteSin, &namelen)==0)
//		RemotePort=TMsgSocket_GetSin(RemoteSin, RemoteAddress, RemotePort);
//}
//---------------------------------------------------------------------------
int TMsgSocket_GetLastSocketError()
{
#ifdef OS_WINDOWS
...
//}
//---------------------------------------------------------------------------
//void TMsgSocket_SetSocket(socket_t s)
//{
//    FSocket=s;
//    if (FSocket!=INVALID_SOCKET)
//    {
...
void TMsgSocket_SetSocketOptions()
{
	...
	int KeepAlive = 1;
		LastTcpError=0;
		TMsgSocket_SockCheck(setsockopt(FSocket, IPPROTO_TCP, TCP_NODELAY,(char*)&NoDelay, sizeof(NoDelay)));

	if (LastTcpError==0)
				TMsgSocket_SockCheck(setsockopt(FSocket, SOL_SOCKET, SO_KEEPALIVE,(char*)&KeepAlive, sizeof(KeepAlive)));
}
//---------------------------------------------------------------------------
int TMsgSocket_SockCheck(int SockResult)
{
...
//---------------------------------------------------------------------------
bool TMsgSocket_CanRead(int Timeout)
{
		struct timeval TimeV;
		int64_t x;
		fd_set FDset;
		...
		if (LastTcpError==0)
		{
			 ...
			 // {
						...
						if (LastTcpError==0)
						{
								...
								if (TMsgSocket_SockCheck(Result)==0)
								{
										TMsgSocket_GetLocal();
...
#endif
//---------------------------------------------------------------------------
void TMsgSocket_SckDisconnect()
{
		TMsgSocket_DestroySocket();
		Connected=false;
}
//---------------------------------------------------------------------------
//void TMsgSocket_ForceClose()
//{
//    if(FSocket != INVALID_SOCKET)
//    {
//        try {
...
//}
//---------------------------------------------------------------------------
//int TMsgSocket_SckListen()
//{
//    LastTcpError=0;
	 // TMsgSocket_SockCheck(listen(FSocket ,SOMAXCONN));
//    return LastTcpError;
//}
//---------------------------------------------------------------------------
//bool TMsgSocket_Ping(char *Host)
//{
//    return Pinger->Ping(Host, PingTimeout);
//}
//---------------------------------------------------------------------------
//bool TMsgSocket_Ping(sockaddr_in Addr)
//{
//	if (PingTimeout == 0)
//		return true;
//	else
...
//}
//---------------------------------------------------------------------------
int TMsgSocket_SendPacket(void *Data, int Size)
{
		int Result;
		...
		if (SendTimeout>0)
		{
				if (!TMsgSocket_CanWrite(SendTimeout))
				{
						LastTcpError = WSAETIMEDOUT;
						return LastTcpError;
				}
...
//{
//    LastTcpError=0;
//    if (TMsgSocket_CanRead(RecvTimeout))
//    {
...
//            else
//                LastTcpError=TMsgSocket_GetLastSocketError(); // we need to know what happened
//    }
//    else
//        LastTcpError = WSAETIMEDOUT;
